...
| Staging  | 🟡 (Orange)            | Currently unused. Mainly for QA to test a release-like app with a staging backend. Imitates the Production/Release application, having features flags following the Prod/White app below, but with extra dev tools. | ✅               | Wire Staging    |
| Alpha    | 🟢 (Green)             | Internal pre-release app channel. Uses the production backend with private-build capabilities enabled. Devs daily driver.                                                                                           | ✅               | Wire Prod       |
