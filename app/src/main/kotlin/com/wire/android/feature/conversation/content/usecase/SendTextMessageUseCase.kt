    ...
    override suspend fun run(params: SendTextMessageUseCaseParams): Flow<Either<Failure, Unit>> = flow {
        suspending {
            ...
            sessionRepository.currentSession().map { session ->
                ...
                val message = Message(
                    UUID.randomUUID().toString(), params.conversationId, session.userId, session.clientId, content,
...
data class SendTextMessageUseCaseParams(val conversationId: String, val text: String)
