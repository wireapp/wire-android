...
class ConversationPrivacyStore(private val context: Context, userId: UserId) {
    ...
    /** Remove orphaned entries for conversations that no longer exist locally. */
    suspend fun retainOnly(existing: Set<ConversationId>) {
        context.dataStore.edit { prefs ->
            val current = prefs[PRIVACY_MAP]?.let(::decode) ?: return@edit
            val pruned = current.filterKeys { it in existing }
            if (pruned.size != current.size) prefs[PRIVACY_MAP] = encode(pruned)
        }
    }
