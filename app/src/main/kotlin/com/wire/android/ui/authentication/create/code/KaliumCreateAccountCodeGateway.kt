...
internal class KaliumCreateAccountCodeGateway(
    ...
    override suspend fun requestActivationCode(serverConfig: ServerConfig.Links, email: String): ActivationCodeRequestResult<CoreFailure> {
    ...
    override suspend fun register(serverConfig: ServerConfig.Links, request: CreateAccountRegistrationRequest): AccountRegistrationResult<CoreFailure, KaliumCreateAccountCredentials> {
    ...
    override suspend fun storeSession(credentials: KaliumCreateAccountCredentials): StoreAccountSessionResult<CoreFailure, UserId> =
        addAuthenticatedUser(credentials.result.toStoreSessionParam(defaultWebSocketEnabledByDefault), replace = false).toStoreSessionResult()
