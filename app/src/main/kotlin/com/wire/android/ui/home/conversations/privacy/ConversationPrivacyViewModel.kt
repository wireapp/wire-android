    ...
    fun onLevelSelected(level: ConversationPrivacyLevel) {
        ...
        viewModelScope.launch {
            if (level == ConversationPrivacyLevel.HIGHLY_SENSITIVE) {
                // Escalating to highly sensitive always requires the Chat PIN passcode: create one if the
                // user has none yet, otherwise confirm with the existing PIN before applying.
