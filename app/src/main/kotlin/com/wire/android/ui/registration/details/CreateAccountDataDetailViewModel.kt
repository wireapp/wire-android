...
/** Metro/Kalium adapter; validation and registration policy live in :features:authentication. */
...
private class KaliumLegacyRegistrationDetailsGateway(
    ...
    override fun isPasswordValid(password: String): Boolean = validatePassword(password).isValid
    ...
    override suspend fun onAccountSetup(withPasswordTries: Boolean) {
        analytics.sendEventIfEnabled(RegistrationPersonalAccount.AccountSetup(withPasswordTries))
    }

    override suspend fun onTermsOfUseDialog() {
        analytics.sendEventIfEnabled(RegistrationPersonalAccount.TermsOfUseDialog)
    }
    ...
    override suspend fun requestActivationCode(
        ...
        val scope = resolveScope(serverConfig) ?: return LegacyActivationCodeResult.AuthScopeUnavailable
