...
class AccessibilityConfigTest : UnitTest() {
    ...
    @Mock
    private lateinit var compatibilityManager: CompatibilityManager

    @Before
    fun setup() {
        accessibilityConfig = AccessibilityConfig(compatibilityManager)
    }

    @Test
    fun `given Android P is required for header compatibility, when build is P or above, then return true`() {
        `when`(compatibilityManager.isCompatibleWith(Build.VERSION_CODES.P)).thenReturn(true)
    ...
    fun `given Android P is required for header compatibility, when build is below P, then return false`() {
        `when`(compatibilityManager.isCompatibleWith(Build.VERSION_CODES.P)).thenReturn(false)
