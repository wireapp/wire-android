...
class EitherTest : UnitTest() {
    ...
    @Test
    fun `given handleFailure is called, when either is Left, invokes the handling function to a new Either`() {
        either = Either.Left(12)
    ...
    @Test
    fun `given handleFailure is called, when either is Left, returns the Either returned by the handle function`() {
        either = Either.Left(12)
        val handledEither = Either.Right("42")

        val result = either.handleFailure {
            handledEither
        }

        result.isRight shouldBe true
        result shouldBeEqualTo handledEither
    }

    @Test
    fun `given handleFailure is called, when either is Left, the left value should be passed correctly to the handle function`() {
        either = Either.Left(12)
    ...
    @Test
    fun `given handleFailure is called, when either is Right, doesn't invoke passed function`() {
        ...
        methodCalled shouldBe false
    }

    @Test
    fun `given handleFailure is called, when either is Right, returns original Either`() {
        either = Either.Right("12")
