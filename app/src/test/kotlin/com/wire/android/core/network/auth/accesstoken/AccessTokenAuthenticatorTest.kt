...
class AccessTokenAuthenticatorTest : UnitTest() {
    ...
    @Test
    fun `Given http cookie header is, when cookie refresh token is not current, then build request with new refresh token`() {
        coEvery { sessionRepository.accessToken(any()) } returns Either.Left(NoEntityFound)
        coEvery { sessionRepository.currentSession() } returns Either.Right(currentSession)
    ...
    @Test
    fun `Given http cookie header is valid, when cookie refresh token is same as now, then build request with current refresh token`() {
        coEvery { sessionRepository.accessToken(any()) } returns Either.Left(NoEntityFound)
        coEvery { sessionRepository.currentSession() } returns Either.Right(currentSession)
