...
class ActivationRemoteDataSourceTest : UnitTest() {
    ...
    @Test
    fun `Given sendEmailActivationCode() is called, when api response is successful, then return success`() = runBlocking {
    ...
    @Test
    fun `Given sendEmailActivationCode() is called, when api response fails, then return a failure`() = runBlocking {
        ...
        verify(activationApi).sendActivationCode(any())
        result.assertLeft { /* no need to check error mapping here */ }
    }
