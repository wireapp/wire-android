...
class RegisterClientUseCaseTest : UnitTest() {
    ...
    @Before
    fun setUp() {
        registerClientUseCase = RegisterClientUseCase(clientRepository, sessionRepository)
    ...
    @Test
    fun `given repository returns authorization token, when registerNewClient returns Failure, then propagate Failure`() {
        val failure = mockk<Failure>()
    ...
    @Test
    fun `given repository returns authorization token, when registerNewClient returns BadRequest, then propagate MalformedPreKeys`() {
    ...
    @Test
    fun `given repository returns authorization token, when registerNewClient returns Forbidden, then propagate DevicesLimitReached`() {
