...
class RegisterPersonalAccountUseCaseTest : UnitTest() {
    ...
    @Before
    fun setUp() {
        useCase = RegisterPersonalAccountUseCase(registrationRepository, activeUserRepository)
    }

    @Test
    fun `given run is called, when registrationRepository and activeUserRepository return success, then returns success`() {
        runBlocking {
            mockRepositoryResponse(Either.Right(TEST_USER_ID))
            `when`(activeUserRepository.saveActiveUser(TEST_USER_ID)).thenReturn(Either.Right(Unit))
            ...
            verify(activeUserRepository).saveActiveUser(TEST_USER_ID)
            result.assertRight()
        }
    }
    ...
    fun `given run is called, when registrationRepository returns Forbidden, then directly returns UnauthorizedEmail`() {
        runBlocking {
            ...
            verifyNoInteractions(activeUserRepository)
        }
    }
    ...
    @Test
    fun `given run is called, when registrationRepository returns success but activeUserRepository fails, then returns that failure`() {
        runBlocking {
            mockRepositoryResponse(Either.Right(TEST_USER_ID))
            val failure = DatabaseFailure()
            `when`(activeUserRepository.saveActiveUser(TEST_USER_ID)).thenReturn(Either.Left(failure))
