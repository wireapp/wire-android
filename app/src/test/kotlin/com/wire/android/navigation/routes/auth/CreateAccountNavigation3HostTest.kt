...
class CreateAccountNavigation3HostTest {
    ...
    fun givenEmailViewModel_whenInspectingHost_thenFeatureBoundaryPreservesContinuationAndHostTypes() {
        ...
        assertTrue(screen.contains("val termsUrl = tosUrl()"))
        assertTrue(screen.contains("CustomTabsHelper.launchUrl(context, termsUrl)"))
    ...
    fun givenCodeViewModel_whenInspectingHost_thenFeatureOwnsStateAndAppKeepsKaliumAndNavigation() {
        ...
        assertTrue(screen.contains("CreateAccountCodeContent("))
        assertTrue(screen.contains("CreateAccountCodePresentation("))
        assertTrue(screen.contains("CreateAccountSummaryNavArgs(flowType)"))
        assertTrue(screen.contains("WireDialog("))
        assertFalse(screen.contains("WireScaffold("))
        ...
        val tooManyDevicesBlock = screen.substringAfter("if (tooManyDevicesError != null)")
        val clearError = tooManyDevicesBlock.indexOf("clearCodeError()")
        val clearField = tooManyDevicesBlock.indexOf("clearCodeField()")
        val removeDevice = tooManyDevicesBlock.indexOf("onTooManyDevices(tooManyDevicesError.userId)")
        assertTrue(clearError >= 0 && clearError < clearField && clearField < removeDevice)
    ...
    fun givenUsernameViewModel_whenInspectingHost_thenFeatureOwnsStateAndSessionHostOwnsAdapters() {
        ...
        assertTrue(screen.contains("CreateAccountUsernameContent("))
        assertTrue(screen.contains("NewAuthContainer("))
        assertTrue(screen.contains("NewAuthHeader("))
        assertTrue(screen.contains("CoreFailureErrorDialog(failure, onDismiss)"))
        assertFalse(screen.contains("toHandleUpdateErrorState"))
        assertFalse(screen.contains("UsernameTextField("))
        assertTrue(featureSources.contains("class CreateAccountUsernameViewModel<FailureT>"))
        assertTrue(featureSources.contains("fun <FailureT> CreateAccountUsernameContent("))
