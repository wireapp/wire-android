...
class CreateAccountCodeGatewayRegistrationTest {
    @Test fun `scope failures map to unavailable without invoking operations`() = runTest {
    ...
    @Test fun `activation request maps every result preserving generic identity`() = runTest {
        val a = createAccountCodeArrangement().withAuthScope(); val failure = NetworkFailure.NoNetworkConnection(null)
        listOf(
            ...
            assertEquals(feature, actual); if (actual is ActivationCodeRequestResult.Generic) assertSame(failure, actual.failure)
        }
    }
    ...
    @Test fun `register resolves scope before reading activation code and maps exact parameters`() = runTest {
        ...
        assertEquals("Alice", actual.firstName); assertEquals("Wire", actual.lastName); assertEquals("secret", actual.password)
        assertEquals("alice@example.com", actual.email); assertEquals("654321", actual.emailActivationCode)
    }
