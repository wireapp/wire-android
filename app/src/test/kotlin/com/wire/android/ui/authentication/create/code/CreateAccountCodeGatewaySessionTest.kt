...
class CreateAccountCodeGatewaySessionTest {
    @Test fun `store uses exact session parameter replace false and maps all failures`() = runTest {
        val a = createAccountCodeArrangement(defaultWebSocket = true)
        val credentials = credentials()
            val parameter = slot<StoreSessionParam>()
        coEvery {
            a.addAuthenticatedUser(
                capture(parameter),
            replace = false
            )
        } returns AddAuthenticatedUserUseCase.Result.Success(TestUser.SELF_USER_ID)
        ...
        coEvery {
            a.addAuthenticatedUser(any(), replace = false)
        } returns AddAuthenticatedUserUseCase.Result.Failure.Generic(failure)
    ...
    @Test fun `client uses exact password model postfix maps results and preserves impossible exceptions`() = runTest {
        ...
        listOf(
            ...
            val thrown = try {
                a.gateway.registerClient(TestUser.SELF_USER_ID, "secret")
                null
            } catch (error: WillNeverOccurError) { error }
