...
class CreateAccountCodeGatewaySessionTest {
    @Test fun `store uses exact session parameter replace false and maps all failures`() = runTest {
        val a = createAccountCodeArrangement(defaultWebSocket = true); val credentials = credentials();
            val parameter = slot<StoreSessionParam>()
        coEvery { a.addAuthenticatedUser(capture(parameter),
            replace = false) } returns AddAuthenticatedUserUseCase.Result.Success(TestUser.SELF_USER_ID)
        ...
        listOf(AddAuthenticatedUserUseCase.Result.Failure.UserAlreadyExists,
            AddAuthenticatedUserUseCase.Result.Failure.SsoIdentityChanged,
                AddAuthenticatedUserUseCase.Result.Failure.NomadSingleUserViolation).forEach { kalium ->
    ...
    @Test fun `client uses exact password model postfix maps results and preserves impossible exceptions`() = runTest {
        val a = createAccountCodeArrangement(buildFlags = CreateAccountCodeBuildFlags(true, "internal", "debug"));
        ...
        assertEquals("secret", parameter.captured.password);
            assertEquals(" [internal_debug]", parameter.captured.modelPostfix); assertNull(parameter.captured.capabilities)
        ...
        val public = createAccountCodeArrangement(buildFlags = CreateAccountCodeBuildFlags(false, "prod", "release"));
