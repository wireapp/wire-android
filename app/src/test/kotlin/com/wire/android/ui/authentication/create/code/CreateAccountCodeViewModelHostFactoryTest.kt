...
class CreateAccountCodeViewModelHostFactoryTest {
    @Test fun `host maps nav input custom default and creates distinct timer per view model`() {
        ...
        assertEquals("Alice", input.firstName)
        assertEquals("Wire", input.lastName)
        assertEquals("secret", input.password)
        assertEquals(true, input.isTeam)
        val timerField = CreateAccountCodeViewModel::class.java.getDeclaredField("resendCodeTimer").apply {
            isAccessible = true
        }
