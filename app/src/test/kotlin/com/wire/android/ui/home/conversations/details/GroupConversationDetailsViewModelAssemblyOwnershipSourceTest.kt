...
class GroupConversationDetailsViewModelAssemblyOwnershipSourceTest {

    @Test
    fun groupConversationDetailsFactoryIsFeatureOwnedAndInstalledOnceByAppComposition() {
        ...
        assertFalse(detailsGraph.contains("groupConversationDetailsViewModel"))
        assertFalse(detailsGraph.contains("GroupConversationDetailsViewModel"))
        assertTrue(featureGraph.contains("object GroupConversationDetailsManualViewModelFactoryGroup"))
        assertTrue(featureGraph.contains("fun groupConversationDetailsViewModel("))
        assertTrue(featureGraph.contains("args: GroupConversationDetailsNavArgs,"))
        assertEquals(
            1,
            Regex("\\bGroupConversationDetailsManualViewModelFactoryMetroBindings::class\\b")
                .findAll(sessionGraph)
                .count(),
        )
        assertTrue(
            detailsEntry.contains("val viewModel = groupConversationDetailsViewModel(route.toViewModelArgs())"),
        )
    }
