...
class CreatePasswordGuestLinkViewModelAssemblyOwnershipSourceTest {

    @Test
    fun createPasswordGuestLinkFactoryIsFeatureOwnedAndInstalledOnceByAppComposition() {
        ...
        assertFalse(detailsGraph.contains("createPasswordGuestLinkViewModel"))
        assertFalse(detailsGraph.contains("CreatePasswordGuestLinkViewModel"))
        assertTrue(featureGraph.contains("object CreatePasswordGuestLinkManualViewModelFactoryGroup"))
        assertTrue(featureGraph.contains("fun createPasswordGuestLinkViewModel("))
        assertTrue(featureGraph.contains("args: CreatePasswordGuestLinkNavArgs,"))
        assertEquals(
            1,
            Regex("\\bCreatePasswordGuestLinkManualViewModelFactoryMetroBindings::class\\b")
                .findAll(sessionGraph)
                .count(),
        )
        assertTrue(
            detailsEntry.contains("viewModel = createPasswordGuestLinkViewModel(it.toViewModelArgs()),"),
        )
    }
