...
class EditSelfDeletingMessagesViewModelAssemblyOwnershipSourceTest {

    @Test
    fun editSelfDeletingMessagesFactoryIsFeatureOwnedAndInstalledOnceByAppComposition() {
        ...
        assertFalse(detailsGraph.contains("editSelfDeletingMessagesViewModel"))
        assertFalse(detailsGraph.contains("EditSelfDeletingMessagesViewModel"))
        assertTrue(featureGraph.contains("object EditSelfDeletingMessagesManualViewModelFactoryGroup"))
        assertTrue(featureGraph.contains("fun editSelfDeletingMessagesViewModel("))
        assertTrue(featureGraph.contains("args: EditSelfDeletingMessagesNavArgs,"))
        assertEquals(
            1,
            Regex("\\bEditSelfDeletingMessagesManualViewModelFactoryMetroBindings::class\\b")
                .findAll(sessionGraph)
                .count(),
        )
        assertTrue(
            detailsEntry.contains("viewModel = editSelfDeletingMessagesViewModel(it.toViewModelArgs()),"),
        )
    }
