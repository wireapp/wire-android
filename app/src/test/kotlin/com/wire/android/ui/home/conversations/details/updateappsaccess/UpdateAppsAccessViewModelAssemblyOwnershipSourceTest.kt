...
class UpdateAppsAccessViewModelAssemblyOwnershipSourceTest {

    @Test
    fun updateAppsAccessFactoryIsFeatureOwnedAndInstalledOnceByAppComposition() {
        ...
        assertFalse(detailsGraph.contains("updateAppsAccessViewModel"))
        assertFalse(detailsGraph.contains("UpdateAppsAccessViewModel"))
        assertTrue(featureGraph.contains("object UpdateAppsAccessManualViewModelFactoryGroup"))
        assertTrue(featureGraph.contains("fun updateAppsAccessViewModel("))
        assertTrue(featureGraph.contains("args: UpdateAppsAccessNavArgs,"))
        assertEquals(
            1,
            Regex("\\bUpdateAppsAccessManualViewModelFactoryMetroBindings::class\\b")
                .findAll(sessionGraph)
                .count(),
        )
        assertTrue(
            detailsEntry.contains("viewModel = updateAppsAccessViewModel(it.toViewModelArgs()),"),
        )
    }
