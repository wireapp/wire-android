...
class UpdateChannelAccessViewModelAssemblyOwnershipSourceTest {

    @Test
    fun updateChannelAccessFactoryIsFeatureOwnedAndInstalledOnceByAppComposition() {
        ...
        assertFalse(detailsGraph.contains("updateChannelAccessViewModel"))
        assertFalse(detailsGraph.contains("UpdateChannelAccessViewModel"))
        assertTrue(featureGraph.contains("object UpdateChannelAccessManualViewModelFactoryGroup"))
        assertTrue(featureGraph.contains("fun updateChannelAccessViewModel("))
        assertTrue(featureGraph.contains("args: UpdateChannelAccessViewModelArgs,"))
        assertEquals(
            1,
            Regex("\\bUpdateChannelAccessManualViewModelFactoryMetroBindings::class\\b")
                .findAll(sessionGraph)
                .count(),
        )
        assertTrue(
            detailsEntry.contains("viewModel = updateChannelAccessViewModel(it.toViewModelArgs()),"),
        )
    }
