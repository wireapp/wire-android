...
| Group conversation options presentation | `GroupConversationOptions`, its 26 dedicated IDs, and all 218 definitions are feature-owned; the reusable item/switch shells are UI-common | App keeps the resource-backed disable dialog and host callbacks; packages and call signatures remain stable |
...
Audited app production-file counts are: conversations **153**, message composer **40**,
...
**27**. The strict app conversations directory has **53** unit tests and **1** Android
 test; **72** files import app `R`, **350** distinct resource-alias `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**136** production files and **52** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
**25** values directories and contain **898** string definitions, including the exact
