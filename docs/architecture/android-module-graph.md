...
| Message-presentation models, primitives, and resources | The single `UIMessage`/`UIQuotedMessage` model closure, quote-content mapping, date grouping, `Copyable`, immutable `MarkdownNode`/`MarkdownPreview`, and the nine message-presentation resource IDs are feature-owned | App keeps `MessageMapper` and subordinate mappers, Markdown parsing/rendering, and Compose/message-list rendering; no parallel model or commonmark dependency is allowed |
...
**27**. The strict app conversations directory has **54** unit tests and **1** Android
 test; **78** files import app `R`, **397** distinct fully-qualified `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**106** production files and **37** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
**25** values directories and contain **232** string definitions, including the exact
**95** localized banner-state definitions. App retains the four banner span-label IDs
with **23** localized definitions. The feature also owns all **120** existing
definitions of the nine message-presentation IDs across their exact qualifiers. The first live internal capability,
