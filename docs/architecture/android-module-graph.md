...
| Message-presentation primitives and resources | Date grouping, the `Copyable` message contract, immutable `MarkdownNode`/`MarkdownPreview` data, and the nine message-presentation resource IDs are feature-owned | App keeps Markdown parsing/rendering and the remaining `UIMessage`/mapper pipeline until the non-breaking-space closure is ready |
...
Audited app production-file counts are: conversations **175**, message composer **40**,
...
**27**. The strict app conversations directory has **54** unit tests and **1** Android
 test; **78** files import app `R`, **404** distinct fully-qualified `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**104** production files and **36** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
