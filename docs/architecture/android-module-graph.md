...
| Conversation media and message-search paging/contracts | `ConversationMediaNavArgs`, `SearchConversationMessagesNavArgs`, package-preserved search/asset paging use cases and `UIPagingItem`, `ConversationAssetMessagesViewModel`/state, `SearchConversationMessagesViewModel`/state, the search no-results renderer with 7 definitions, the media empty-state renderer with 14 definitions, and the dedicated assisted Metro gateways are feature-owned | App keeps Navigation 3 entries/mappers, the search route/screen, and installs each generated binding exactly once in session composition; remaining media host adapters stay app-owned |
| Record-audio information presentation | Package-preserved `RecordAudioInfoMessageType` and its 2 IDs/14 exact localized definitions are feature-owned | App keeps `EnabledMessageComposer`, Android toast dispatch, recording/runtime permissions, and other host composer behavior |
| Muted conversation badge | Package-preserved `MutedConversationBadge`, its accessibility ID with 17 exact localized definitions, and byte-identical `ic_mute` drawable are feature-owned | App keeps conversation-list factories and the conversation bottom sheet; its remaining icon consumer uses the feature resource namespace |
...
| Message-presentation models, actions, chrome, mappers, and resources | The single `UIMessage`/`UIQuotedMessage` model closure, quote-content mapping, package-preserved `MessageClickActions`, public package-preserved `MessageBody.shouldHideStandalonePreviewedUrl`, message bubble, status indicator, system-message leading/content contract, group avatar, author, reaction, regular-message leading, offline paging, self-deletion timer state, icon metrics and renderer, date grouping, `Copyable`, immutable `MarkdownNode`/`MarkdownPreview`, `MessageResourceProvider`, `SystemMessageContentMapper`, `RegularMessageMapper`, `MessageContentMapper`, `MessageMapper`, `MessagePreviewContentMapper`, `ISOFormatter`, all 59 provider/model/mapper resource IDs, 2 reaction accessibility IDs, 3 status accessibility IDs, 3 dedicated status drawables, 10 timer plural IDs, and 4 expiration-renderer IDs are feature-owned; neutral `UiTextResolver` and 2 cross-feature/app status drawables remain in `:core:ui-common` | App keeps resource-backed system-message factories, Markdown parsing/rendering, remaining Compose/message-list rendering, and all action consumers; no parallel model, chrome, action contract, mapper, formatter, or commonmark dependency is allowed |
...
| Group conversation options and guest-link presentation | `GroupConversationOptions`, `SelfDeletingMessageOption`, `PasswordProtectedLinkBanner`, the guest-link action buttons, their 34 dedicated IDs, and all 291 definitions are feature-owned; the reusable item/switch shells are UI-common | App keeps the resource-backed disable dialog, clipboard/share intents, host callbacks, and guest-access screens; packages and call signatures remain stable |
...
59 message-presentation, provider, mapper, and formatting IDs across their exact qualifiers, plus all **6** definitions of the 2 reaction accessibility IDs, all **34** definitions of the 3 status accessibility IDs, 3 dedicated byte-identical status drawables, all **65** definitions of the 10 self-deletion timer plural IDs, all **31** existing definitions of the 4 expiration-renderer IDs, all **218** definitions of the 26 group-options IDs, all **13** definitions of the 2 self-deletion-option IDs, all **7** definitions of the search no-results ID, all **15** definitions of the 2 password-protected-link banner IDs, all **14** definitions of the 2 record-audio failure IDs, all **45** definitions of the 4 guest-link action IDs, all **14** definitions of the 2 media empty-state IDs, and all **17** definitions of the muted-conversation accessibility ID plus byte-identical `ic_mute`. UI common owns the other 2 byte-identical status drawables used outside conversation. The first live internal capability,
