...
**Last verified:** 2026-08-24, `chore/android-modularization`, baseline HEAD `fbc1961ab`.
...
| Group conversation options and guest-link presentation | `GroupConversationOptions`, `SelfDeletingMessageOption`, `PasswordProtectedLinkBanner`, their 30 dedicated IDs, and all 246 definitions are feature-owned; the reusable item/switch shells are UI-common | App keeps the resource-backed disable dialog and host callbacks; packages and call signatures remain stable |
...
Audited app production-file counts are: conversations **147**, message composer **39**,
...
**27**. The strict app conversations directory has **54** unit tests and **1** Android
 test; **69** files import app `R`, **345** distinct resource-alias `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**143** production files and **56** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
**25** values directories and contain **947** string definitions, including the exact
