...
**Last verified:** 2026-08-24, `chore/android-modularization`, baseline HEAD `de830886e2e5297b22234a4088ded89454e6f098`.
...
| Message-presentation models, primitives, mappers, and resources | The single `UIMessage`/`UIQuotedMessage` model closure, quote-content mapping, date grouping, `Copyable`, immutable `MarkdownNode`/`MarkdownPreview`, `MessageResourceProvider`, `SystemMessageContentMapper`, `ISOFormatter`, and all 17 provider/model/mapper resource IDs are feature-owned | App keeps the regular/content/preview/final message mappers, Markdown parsing/rendering, and Compose/message-list rendering; no parallel model, formatter, or commonmark dependency is allowed |
...
**109** production files and **39** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
