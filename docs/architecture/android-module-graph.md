...
Audited app production-file counts are: conversations **154**, message composer **40**,
...
**27**. The strict app conversations directory has **53** unit tests and **1** Android
 test; **73** files import app `R`, **372** distinct resource-alias `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**135** production files and **51** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
**25** values directories and contain **680** string definitions, including the exact
