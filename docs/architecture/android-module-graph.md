...
Audited app production-file counts are: conversations **155**, message composer **40**,
...
**27**. The strict app conversations directory has **53** unit tests and **1** Android
 test; **74** files import app `R`, **377** distinct resource-alias `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**134** production files and **50** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
**25** values directories and contain **649** string definitions, including the exact
