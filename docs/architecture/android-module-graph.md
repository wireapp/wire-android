...
| Message-presentation primitives | Date grouping, the `Copyable` message contract, and immutable `MarkdownNode`/`MarkdownPreview` data are feature-owned with package/FQN preservation | App keeps Markdown parsing/rendering, resources, and the remaining `UIMessage`/mapper pipeline until its closure is ready |
...
**104** production files and **35** unit-test files. Its **9** Crowdin-tracked `strings.xml` files span
