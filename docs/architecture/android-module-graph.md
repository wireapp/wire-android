...
| Group conversation options presentation | `GroupConversationOptions`, `SelfDeletingMessageOption`, their 28 dedicated IDs, and all 231 definitions are feature-owned; the reusable item/switch shells are UI-common | App keeps the resource-backed disable dialog and host callbacks; packages and call signatures remain stable |
...
Audited app production-file counts are: conversations **148**, message composer **40**,
...
**27**. The strict app conversations directory has **54** unit tests and **1** Android
 test; **70** files import app `R`, **347** distinct resource-alias `R.type.name`
IDs occur there, and **3** files use `BuildConfig`. `:features:conversation` now owns
**141** production files and **54** unit-test files. Its **25** Crowdin-tracked `strings.xml` files span
**25** values directories and contain **918** string definitions, including the exact
