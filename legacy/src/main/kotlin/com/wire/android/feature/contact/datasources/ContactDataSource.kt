...
class ContactDataSource(
    ...
    override suspend fun cachedDetailedContactsById(ids: Set<String>): Either<Failure, List<DetailedContact>> = suspending {
        contactLocalDataSource.contactClientsByContactId(ids)
                .map { it.map(contactMapper::fromContactWithClients) }
    }

    override suspend fun addNewClientsToContact(contactId: String, clients: List<ContactClient>): Either<Failure, Unit> = suspending {
        contactsById(setOf(contactId)).map { it.first() }
                .map { contactMapper.fromDetailedContactToContactWithClients(DetailedContact(it, clients)) }
                .flatMap { contactLocalDataSource.saveNewClients(it.clients) }
    }
}
