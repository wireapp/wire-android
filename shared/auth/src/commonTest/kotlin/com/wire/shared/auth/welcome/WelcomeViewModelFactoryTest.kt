...
import com.wire.shared.auth.login.model.LoginApiProxy
...
class WelcomeViewModelFactoryTest {
    @Test
    fun givenConfig_whenCreatingViewModel_thenStateUsesConfig() {
        val viewModel = WelcomeViewModelFactory(
    ...
    fun givenLoginClicked_whenSendingIntent_thenNavigateToLoginEffectIsEmitted() = runTest {
        val viewModel = WelcomeViewModelFactory(SharedAuthConfig(serverLinks)).create()
    ...
    fun givenProxyConfigured_whenCreatingPersonalAccount_thenProxyLimitationEffectIsEmitted() = runTest {
        ...
        val viewModel = WelcomeViewModelFactory(SharedAuthConfig(links)).create()
        val effect = async(start = CoroutineStart.UNDISPATCHED) { viewModel.effects.first() }
