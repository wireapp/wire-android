...
/**
 * Builds [TestinyRuntimeConfig] from the instrumentation args added by run_ui_tests.sh.
 * The B64 args are decoded here before the reporter uses them.
 */
object TestinyRuntimeConfigResolver {
    ...
    fun fromInstrumentationArgs(arguments: Bundle = InstrumentationRegistry.getArguments()): TestinyRuntimeConfig? {
        ...
        // No Testiny args means reporting is off for this run.
    ...
    private fun Bundle.readDecodedTrimmed(key: String): String? =
        readTrimmed(key)?.let { encodedValue ->
            // run_ui_tests.sh uses B64 args to avoid shell parsing issues.
