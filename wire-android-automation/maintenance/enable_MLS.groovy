// Description: This pipeline is used to enable MLS feature for a team.
node('Job_distributor') {

    env.BackendType = params.BackendType
    env.TeamID = params.TeamID
    env.defaultCipherSuite = params.defaultCipherSuite
    env.allowedCipherSuites = params.allowedCipherSuites
    env.defaultProtocol = params.defaultProtocol
    env.supportedProtocols = params.supportedProtocols
    ...
    withMaven(jdk: 'JDK17', maven: 'M3') {
        ...
        stage('Run maintenance tool') {
            ...
            withCredentials([
                ...
                stage('Run maintenance tool') {
                    ansiColor('xterm') {
                        sh 'cd "$WORKSPACE/tests/maintenance" && java -jar target/maintenance-cli-jar-with-dependencies.jar enableMLSFeatureTeam ${BackendType} ${TeamID} ${defaultCipherSuite} ${allowedCipherSuites} ${defaultProtocol} ${supportedProtocols}'
                    }
                }
            }
        }
