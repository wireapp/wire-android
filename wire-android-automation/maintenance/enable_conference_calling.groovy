properties([
        parameters([
                ...
                string(description: 'The email of the team owner', name: 'UserEmail', trim: true),
                string(description: 'The password of the team owner', name: 'UserPassword', trim: true),
                booleanParam(defaultValue: true, description: 'Check this to enable conference calling for the given team instead', name: 'Enable')
        ])
])
...
node('Job_distributor') {
    ...
    withMaven(jdk: 'JDK17', maven: 'M3') {
        ...
        stage('Run maintenance tool') {
            ansiColor('xterm') {
                sh 'cd $WORKSPACE/tests/maintenance && java -jar target/maintenance-cli-jar-with-dependencies.jar changeConferenceCalling ${BackendType} ${UserEmail} ${UserPassword} ${Enable}'
            }
        }
    }
}
